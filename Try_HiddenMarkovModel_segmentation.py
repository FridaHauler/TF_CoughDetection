...
# for using pywt install: conda install -c conda-forge pywavelets
...
def trainModel(seq,label,model=None):
    ...
    if model is None:
        ...
        verbose=True)
        print("trnas_mat: ")
        print(ret_model.dense_transition_matrix())
        print("model:")
        print(ret_model)
        #ret_model.dense_transition_matrix()[3][4] = 0.005
        #mat[3][4] = 0.005
        ret_model.freeze()
        ret_model.bake()
        #ret_model.plot()
        ...
        #model=model.from_samples(NormalDistribution,n_components=7,
        #                         X=seq,verbose=True,max_iterations=1)
...
def handle_missing_labels(record_missing_labels, master_record,\
    ...
    for item in record_missing_labels:
        if item[1] == 'p_onset':
            ...
            ind = master_file_index[item[0]]
            print('what is q_onset:', q_onset, '*********', q_onset_loc)
...
def plot_dist(master_record, master_label, master_file_list, log):
    ...
    print("Am I at distributions?")
    for dist in distributions:
        print("done1 distributions", dist)
        sns.distplot(dist, 20, hist=False,rug=True,label='dist')
    print("done with distplot")
    plt.legend()
    plt.show()

def update_labels_to_digits(master_label):
    mapping = {"Single cough":"s0","Multiple Coughs":"s1","noLabel":"s2","Clear troat":"s3","Laugh":"s4"}
    for curr_list  in master_label:
        ind = 0
...
def create_label_blocks(master_label):
    ...
    for list1 in master_label:
        start = list1.index("noLabel")
        end = list1.index("s_end")
        for i in range(0, start):
            list1[i] = "Single cough"
        for i in range(start,end+1):
            list1[i] = "Multiple Coughs"
    ...
    for i in range(0, num_iterations):
        ...
        test_files):
            ...
            (4 not in prediction)):
                ...
                noLabel = record[0 : prediction.index(1)]
                noLabel_loc = generate_loc(0, prediction.index(1))
                ...
                plt.plot(noLabel_loc,noLabel,color="red",label="noLabel")
                ...
                #-------------------------------------------------------
                noLabelt = orig_record[0 : curr_test_label.index(1)]
                noLabelt_loc = generate_loc(0, curr_test_label.index(1))
                ...
                plt.plot(noLabelt_loc,noLabelt,color="red", label ="noLabelt")
...
def new_plot(prediction, record, orig_record, temp_label, filename):
    ...
    if((0 not in prediction) or (1 not in prediction) or (2 not in prediction) or (3 not in prediction) or (4 not in prediction)):
        ...
        #plt.legend()
        #plt.plot()
        plt.savefig(".\\TEST5\\"+filename+".png",bbox_inches="tight")
        #plt.show()
        plt.close()
        return 1
...
def convert_label(target_label):
    for ind, item in enumerate(target_label):
        if item == "Single cough" or item == "s0":
            target_label[ind] = 0
        elif item == "Multiple Cough" or item == "s1":
            target_label[ind] = 1
        elif item == "noLabel" or item == "s2":
            target_label[ind] = 2
        elif item == "Clear troat" or item == "s3":
            target_label[ind] = 3
        elif item == "Laugh" or item == "s4":
            target_label[ind] = 4
    return target_label
...
def explore_average_lengths(trunc_master_label):
    ...
    print()
    print("Average legth of B1/SingleCough: ", np.average(len1))
    print("Average legth of QRS/MultipleCough: ", np.average(len2))
    print("Average legth of B2/noLabel: ", np.average(len3))
    print("Average legth of T/ClearThroat: ", np.average(len4))
    print("Average legth of B3/Laughs: ", np.average(len5))
...
def compute_dice_coefficient(all_test_predictions, num_iterations, test_label):
    dice_noLabel, dice_qrs, dice_b2, dice_t, dice_b3 = ([] for i in range (5))
    all_dice = [dice_noLabel, dice_qrs, dice_b2, dice_t, dice_b3]
    for i in range(0, num_iterations):
        ...
        for prediction, target in zip(curr_prediction_list, test_label):
            ...
            seg_noLabel_pred = [0, prediction.index(1)-1]
            seg_noLabel_target = [0, conv_target_label.index(1)-1]
            ...
            pred_seg_list = [seg_noLabel_pred, seg_qrs_pred, seg_b2_pred, seg_t_pred, seg_b3_pred]
            tar_seg_list = [seg_noLabel_target, seg_qrs_target, seg_b2_target,  seg_t_target, seg_b3_target] 
            for i in range(0, 5):
    ...
    print()
    print("Dice similarity for Single cough: ", np.average(all_dice[0]))
    print("Dice similarity for MultipleCoughs: ", np.average(all_dice[1]))
    print("Dice similarity for noLabel: ", np.average(all_dice[2]))
    print("Dice similarity for Clear throat: ", np.average(all_dice[3]))
    print("Dice similarity for Laughs: ", np.average(all_dice[4]))
...
def readfiles(mypath):
    ...
    #check = 0
    #for i in range(0,len(master_record[0])):
    #    if (backup_record[0][i]) != (trans_master_record[0][i]):
    #        check = 1
    #        print("DIFFERENT!!!!")
    #        break
    ...
    print("Creating model from custom parameters...")
    trained_model = try_custom_model()
    ...
    print("Single cough train accuracy: ", cluster_accuracies[0]*100,"%") 
    print("MultipleCoughs train accuracy: ", cluster_accuracies[1]*100,"%")
    print("noLabel train accuracy: ", cluster_accuracies[2]*100,"%")
    print("Clear throat train accuracy: ", cluster_accuracies[3]*100,"%")
    print("Laughs train accuracy: ", cluster_accuracies[4]*100,"%")
    print()
    ...
    print("Single cough test accuracy: ", cluster_accuracies[0]*100,"%")
    print("MultipleCoughs test accuracy: ", cluster_accuracies[1]*100,"%")
    print("noLabel test accuracy: ", cluster_accuracies[2]*100,"%")
    print("Clear throat test accuracy: ", cluster_accuracies[3]*100,"%")
    print("Laughs test accuracy: ", cluster_accuracies[4]*100,"%")
    
    compute_dice_coefficient(all_test_predictions, num_iterations, test_label)
    
    #explore_average_lengths(trunc_master_label)
